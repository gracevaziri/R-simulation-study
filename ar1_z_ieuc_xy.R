#simulates a regular grid with isotropic euclidian correlation in the x and y directions, 
...
xy.phi <- 0.3 #exponential autocorrelation across xy
...
asreml.fit <- asreml(fixed = l.obs ~ z, random =~ spl(z) + stn, data = glm.spl, 
                     splinepoints = list(z = seq(0, 250, 25)), rcov=~ ar1(z.fact):ieuc(x.fact, y.fact), maxIter = 50)
summary(asreml.fit)
