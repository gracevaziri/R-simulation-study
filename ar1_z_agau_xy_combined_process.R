...
y.phi <- 0.3 #autocorrelation across y
...
for (i in 1:max(y)) {
  for (k in 2:length(z)) {  
    for (j in 1:max(x)) {
      ...
      t.cor[w] <- t.cor[y == i & z.int == (k - 1) & x == j]*phi.true + sum((matrix(rep(cor.ij[c(1:max(x)), j], max(y)), ncol = max(x), byrow = T) * matrix(rep(cor.ij.y[c(1:max(y)), i], max(x)), ncol = max(x)))*matrix(r.noise[z.int == k], ncol = max(x), byrow = T))
    }
  }
}
...
vals <- matrix(c(stn.sd, noise.sd, phi.true, x.phi, x.phi, round(summary(asreml.fit)$varcomp[2,2]^0.5, 2), round(summary(asreml.fit)$varcomp[3,2]^0.5, 2), round(summary(asreml.fit)$varcomp[4,2], 2), round(summary(asreml.fit)$varcomp[5,2], 2), round(summary(asreml.fit)$varcomp[6,2], 2)), ncol = 2)
colnames(vals) <- c("true", "fitted")
rownames(vals) <- c("stn", "noise", "z ar1", "y agau", "x agau")
vals
