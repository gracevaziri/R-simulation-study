...
for (i in 1:max(y)) {
  t.cor[y == i & z.int == 1] <- rnorm(max(y), 0, noise.sd)
  t.cor[y == i & x == 1] <- rnorm(length(z), 0, noise.sd) 
  for (k in 2:length(z)) {  
    for (j in 2:max(x)) {
      w <- which(y == i & z.int ==k & x == j)
      t.cor[w] <- t.cor[y == i & z.int == (k - 1) & x == j]*phi.true + t.cor[y == i & z.int == k & x == (j - 1)]*x.phi + t.cor[y == i & z.int ==(k - 1) & x == (j - 1)]*(-phi.true*x.phi) + rnorm(1, 0, noise.sd)
    }
  }
}
