...
xy.phi <- 0.2 #exponential autocorrelation across xy
...
for (i in 1:100) {
  for (k in 1:100) {
    ...
    dist[i, k] <- abs(w_current[1] - w_new[1]) + abs(w_current[2] - w_new[2])      
  }
}
...
asreml.fit <- asreml(fixed = l.obs ~ z, random =~ spl(z) + stn, data = glm.spl, 
                     splinepoints = list(z = seq(0, 250, 25)), rcov=~ ar1(z.fact):iexp(x.fact, y.fact), maxIter = 50)
summary(asreml.fit)
