#simulates an irregular grid with isotropic gaussian correlation in the x and y directions, 
...
xy.phi <- 0.5 #euclidian autocorrelation across x
...
dist <- matrix(0, ncol = n.station, nrow = n.station)
for (i in 1:n.station) {
  for (k in 1:n.station) {    
    dist[i, k] <- (x[i] - x[k])^2 + (y[i] - y[k])^2     
  }
}
...
asreml.fit <- asreml(fixed = l.obs ~ z, random =~ spl(z) + stn, data = glm.spl, 
                     splinepoints = list(z = seq(0, 250, 25)), rcov=~ ar1(z.fact):igau(x, y), maxIter = 50)
summary(asreml.fit)
